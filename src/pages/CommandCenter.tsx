...
export function CommandCenter() {
    const { refreshStorageStats, hydrate, activeChatId, isHydrated, isOnboardingOpen, isLocked } = useVortexStore();
    ...
    return (
        <div className="w-full h-screen flex overflow-hidden bg-bg-primary relative">

            {/* Main App Content - Only render if fully authenticated and setup */}
            {isHydrated && !isLocked && !isOnboardingOpen && (
                <>
                    {/* Left: Peer List */}
                    <div
                        className={`
            ...
            <AnimatePresence>
                {isHydrated && isLocked && <LockScreen />}
            </AnimatePresence>

            {/* Onboarding Wizard */}
            <AnimatePresence>
                {isHydrated && !isLocked && isOnboardingOpen && <OnboardingModal />}
            </AnimatePresence>
        </div>
    );
}
